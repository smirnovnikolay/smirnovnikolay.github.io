 jQuery(function($){
  $(document).mouseup(function (e){ // событие клика по веб-документу
    ...
    //popup basket close
    var div = $(".stash__pop"); // тут указываем ID элемента
    ...
    //popup option close
    var div = $(".popup-option__item"); // тут указываем ID элемента

    if (!div.is(e.target) // если клик был не по нашему блоку
        && div.has(e.target).length === 0) { // и не по его дочерним элементам
      div.parents('.popup-option__value').removeClass('active'); // скрываем его
    }
    ...
    if (!div.is(e.target) // если клик был не по нашему блоку
        && div.has(e.target).length === 0) { // и не по его дочерним элементам
      $('.menu__wrapper').removeClass('active'); // скрываем его
...
//
 ...
 $(function() {
    $('#phone1, #phone2, #phone3, #phone4').mask("+380 (999) 999-99-99",{autoclear: false});    
  ...
  function windowSize(){
        ...
        variableWidth: true,
          ...
          $(!'.stash__pop').on("click", function(){
            $('.stash__pop').removeClass('active');              
          }); 
